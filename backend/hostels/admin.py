...
class HostelSubscriptionAdmin(admin.ModelAdmin):
    list_display = ['caretaker', 'hostel', 'amount_paid', 'paid_via', 'status', 'start_date', 'end_date']
    list_filter = ['status', 'paid_via', 'created_at']
    search_fields = ['caretaker__name', 'caretaker__email', 'hostel__name']
    readonly_fields = ['created_at', 'updated_at']

    def get_queryset(self, request):
        return super().get_queryset(request).select_related('caretaker', 'hostel')
