    ...
    const handleSaveRoom = async (roomData) => {
        try {
            ...
            Object.keys(roomData).forEach(key => {
                if (key !== 'image' && roomData[key] !== null && roomData[key] !== undefined) {
                    let value = roomData[key];
                    if (key === 'is_available') {
                        value = roomData[key] ? 'true' : 'false';
                    }
                    if (key === 'hostel') {
                        value = parseInt(roomData[key], 10);
                    }
            ...
            if (editingRoom) {
                const updatedRoom = await api.upload(API_CONFIG.ROOMS.DETAIL(editingRoom.id), formData, 'PATCH');
                setRooms(rooms.map(r => r.id === editingRoom.id ? updatedRoom : r));
                alert('Room updated successfully');
            } else {
                const newRoom = await api.upload(API_CONFIG.ROOMS.CREATE, formData, 'POST');
                ...
                <div className="admin-tab-content active">
                    ...
                    <table className="admin-table">
                        ...
                        <tbody>
                            {users.filter(u => displayUserName(u).toLowerCase().includes(studentSearchTerm.toLowerCase())).map(u => {
                                ...
                                return (
                                    <tr key={u.id}>
                                        ...
                                        <td style={{ fontFamily: 'monospace', fontWeight: '500' }}>{u.student_reg_no || 'N/A'}</td>
                                        <td>{u.email}</td>
                                        <td>{u.gender}</td>
                            ...
                            {users.length === 0 && <tr><td colSpan="8" className="text-center" style={{color:'#64748b', textAlign: 'center'}}>No students registered yet</td></tr>}
                        </tbody>
                    </table>
                </div>
            );
                        ...
                        <tbody>
                            {reservations.filter(res => res.reservation_code.toLowerCase().includes(reservationSearchTerm.toLowerCase()) || res.student.toLowerCase().includes(reservationSearchTerm.toLowerCase())).map(reservation => (
                                <tr key={reservation.id}>
                                    ...
                                    <td>{reservation.booking_date || reservation.check_in_date || 'N/A'}</td>
                ...
                <div className="admin-tab-content active">
                    ...
                    {loading ? (
                        ...
                        <table className="admin-table">
                            ...
                            <tbody>
                                {caretakers.filter(c => displayUserName(c).toLowerCase().includes(caretakerSearchTerm.toLowerCase())).map(caretaker => (
                                    <tr key={caretaker.id}>
                    ...
                    {!selectedManageHostel ? (
                        ...
                        <div style={{ marginTop: '1.5rem' }}>
                            ...
                            <table className="admin-table">
                                ...
                                <tbody>
                                    {filteredRooms.filter(r => r.room_number.toLowerCase().includes(roomSearchTerm.toLowerCase())).map(r => (
                                        <tr key={r.id}>
                                            <td>
                                                {r.image && !r.image.includes('placeholder.jpg') ? (
                                                    <img src={r.image} alt={r.room_number} style={{ width: '40px', height: '40px', objectFit: 'cover', borderRadius: '4px' }} />
                                                ) : (
                                                    <div style={{ width: '40px', height: '40px', background: '#e2e8f0', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.6rem', color: '#94a3b8' }}>No Img</div>
                                                )}
                                            </td>
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <form onSubmit={handleSubmit}>
                        <div style={{ maxHeight: 'calc(90vh - 150px)', overflowY: 'auto', paddingRight: '0.5rem', marginBottom: '1rem' }}>
                ...
                <div style={{
                    ...
                    <form onSubmit={handleSubmit}>
                        <div style={{ maxHeight: 'calc(90vh - 150px)', overflowY: 'auto', paddingRight: '0.5rem', marginBottom: '1rem' }}>
                            ...
                            <div style={{marginBottom: '1rem'}}>
                                <label style={{display: 'block', marginBottom: '0.5rem'}}>Room Image</label>
                                <input
                                    type="file" accept="image/*"
                                    onChange={(e) => setFormData({...formData, image: e.target.files[0]})}
                                    style={{width: '100%', padding: '0.5rem', border: '1px solid #ddd', borderRadius: '4px'}}
                        ...
                        <div style={{
                            ...
                            paddingTop: '1rem', borderTop: '1px solid #eee'
                        }}>
